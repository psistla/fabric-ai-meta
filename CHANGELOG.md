...
### Added
- `guide_query(model, *, measure=None, column=None, dimensions=None) -> dict` under `fabric_ai_meta.analyzer.query_guidance`, and the `guide_query` MCP tool. Pass exactly one of a measure name or a raw column about to be aggregated directly; get back the correct measure to use instead, a safe join path to the requested dimensions, and warnings for semi-additive, ratio, hardcoded-literal, and calculation-group traps. Refuses cleanly (not silently) when both or neither of `measure`/`column` is given.
- `generate_capability_manifest(model) -> dict` under `fabric_ai_meta.analyzer.capability_manifest`, and `fabric-ai-meta export capability-manifest`. Classifies every measure in a model as `answerable`, `answerable_with_caveats` (same warning set as `guide_query`, read once for the whole model), or `refused` (report plumbing: an icon, a hardcoded display string, a `FORMAT()`-built title). File-based export only; no MCP tool yet.
- `assess_agent_readiness(model) -> dict` under `fabric_ai_meta.analyzer.agent_readiness`, `fabric-ai-meta export agent-readiness`, and the `assess_agent_readiness` MCP tool. A per-model critic report: ranked findings (`undescribed`, `ambiguous_name`, `missing_relationship`, `unreliable_type`), each paired with a suggested fix and weighted by how much it affects the model's readiness score. The "fix it first" companion to `score`: `score` gives the number, this gives what to change to move it.
- `fabric-ai-meta serve` now exposes eight MCP tools (was six): `guide_query` and `assess_agent_readiness` are new, alongside the existing `list_models`, `analyze_model`, `score_model`, `generate_schema`, `governance_report`, and `diff_summaries`.
- New JSON schemas `schemas/capability-manifest/v1.json` and `schemas/agent-readiness/v1.json`, both pinned against real `--pbip` fixtures rather than hand-authored data.
- `__all__` grows from 47 to 51: `guide_query`, `generate_capability_manifest`, `assess_agent_readiness`, and `write_agent_readiness_report`.

### Fixed
- `--pbip` now parses TMDL's three-backtick verbatim delimiter for a measure's DAX. TMDL emits this fence when an expression must be read literally; `_parse_measure` had only ever handled the single-line and indentation-continuation forms, so a fenced measure's `dax` field carried the closing fence line as part of the expression, or extracted nothing.
- `--pbip` now splits a calculated column's declared name from its DAX, the same way `_parse_measure` already did for measures. `_parse_column` never applied that split, so a calculated column's `name` field carried the whole `'Sales amt' = RANDBETWEEN(10,1000)` header, DAX and all, corrupting every feature that reads calculated-column names (`guide_query`, `capability-manifest`, `governance`, `score`).
- Sort columns are now detected from the `sortByColumn` property Power BI declares, rather than guessed from the name. A column is a sort column when another column in the table names it as its sort-by target. The `--pbip` parser was discarding the property entirely. The name fallback remains for models that ship `*Sort` columns without wiring the property up, but `order` is no longer a keyword: it matched `Order ID`, `Order Status` and `Purchase Order Number` in any sales model, none of which are sort columns. The old rule also excluded numeric types, which meant it could never match a real sort column, since those are nearly always integers.
...
- `networkx` and `pydantic` are no longer dependencies. Neither was imported anywhere in the package; the DAX dependency graph is built with plain dictionaries and every model is a stdlib `dataclass`.
